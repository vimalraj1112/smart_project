...
const StatCard = ({ label, value, sub, icon: Icon }) => (
  <div
    className="bg-white/[0.04] border border-white/[0.08] rounded-xl px-5 py-4 backdrop-blur-md flex items-center justify-between 
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`px-2 py-1 rounded-md text-[11px] font-medium transition ${styles[status]}`}
    >
      {label[status]}
    </span>
  );
...
export default function Dashboard() {
  ...
  if (loading)
    return <div className="text-white/40 p-10 animate-pulse">Loading…</div>;
  if (!data)
    return <div className="text-red-400 p-10">Failed to load dashboard.</div>;

  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white px-6 py-10">
      {/* Header */}
      <div className="mb-8 animate-fade-in">
        <h1 className="text-3xl font-bold mb-2">Good day, {user.name}</h1>
