...
import {
  Plus, Search, Users, Trash2, Pencil, UserPlus, FolderKanban
} from 'lucide-react'
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="w-full max-w-sm bg-white/5 border border-white/10 backdrop-blur-xl rounded-2xl p-6">
        ...
        <select
          ...
          className="w-full bg-white/[0.05] border border-white/10 text-white rounded-xl px-4 py-3 text-sm mb-5"
        >
    ...
    <div className="min-h-screen bg-[#0a0a0f] p-6 text-white">
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={p.id} className="border-t border-white/5 hover:bg-white/[0.03]">
                ...
                <td className="p-4">
                  <Link to={`/projects/${p.id}`} className="text-indigo-400 hover:underline">
                    {p.project_name}
                  </Link>
                </td>
      ...
      {memberProject && (
        <MemberModal
          ...
          onSaved={() => { setMemberProject(null); fetch() }}
        />
      )}
    </div>
  )
